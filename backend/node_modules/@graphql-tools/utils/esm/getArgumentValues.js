...
import { isNonNullType, Kind, print, valueFromAST, valueFromASTUntyped, } from 'graphql';
...
export function getArgumentValues(def, node, variableValues = {}) {
    ...
    for (const arg of def.args) {
        ...
        if (!argumentNode) {
            if ('default' in arg && arg.default) {
                ...
                coercedValues[arg.name] =
                    'value' in arg.default
                        ? arg.default.value
                        : valueFromASTUntyped(arg.default.literal);
            }
                ...
                throw createGraphQLError(`Argument "${arg.name}" of required type "${inspect(arg.type)}" ` + 'was not provided.', {
                    nodes: [node],
                });
            }
            continue;
        ...
        if (valueNode.kind === Kind.VARIABLE) {
            ...
            if (variableValues == null || !hasOwnProperty(variableValues, variableName)) {
                if (arg.defaultValue !== undefined) {
                    ...
                    throw createGraphQLError(`Argument "${arg.name}" of required type "${inspect(arg.type)}" ` +
        ...
        if (isNull && isNonNullType(arg.type)) {
            throw createGraphQLError(`Argument "${arg.name}" of non-null type "${inspect(arg.type)}" ` + 'must not be null.', {
                nodes: [valueNode],
            });
        }
